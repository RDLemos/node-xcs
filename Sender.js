...
function Sender(config) {
    if (config && util.isNullOrUndefined(config.senderID || util.isNullOrUndefined(config.serverKey))) {
        throw new IllegalArgumentException();
    }
    this.senderId = config.senderId;
    this.serverKey = config.serverKey;
    this.connIdx = config.connIdx;
	...
	// Set default to Production port
    ...
    this.client.on('online', function () {
        self.events.emit('connected', self.connIdx);
    ...
    this.client.on('close', function () {
        if (self.draining) {
            ...
            self.events.emit('disconnected', self.connIdx);
        }
    });
